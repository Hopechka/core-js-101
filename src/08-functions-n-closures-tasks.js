...
function getComposition(f, g) {
  return (x) => [g, f].reduce((prev, cur) => cur(prev), x);
}
...
function getPolynom(...args) {
  ...
  const reversArgs = arr.concat([0, 0, 0]);
  return (x) => {
    const res = reversArgs[0] + reversArgs[1] * x + reversArgs[2] * x ** 2;
...
function memoize(func) {
  const cache = [];
  return () => {
    if (cache.length === 0) { cache.push(func()); } return cache[0];
  };
}
...
function retry(func, attempts) {
  ...
  return function _retry() {
    try {
      ...
      if (count < attempts) { count += 1; return _retry(); }
    }
...
function getIdGeneratorFunction(startFrom) {
  let count = startFrom;
  return () => {
    if (count === startFrom) { count += 1; return startFrom; }
    const boo = count;
    count += 1;
    return boo;
  };
